#!/usr/bin/env bash
# Tmux session: rb10_1300e real robot + ZED Mini + RTAB-Map (localization).
#
# Layout:
#   ┌──────────────────────┬──────────────────────────┐
#   │  [0] Robot bringup   │                          │
...
#
# Usage:
#   ./slam_session_localization_real_robot.sh [robot_ip] [odom_source] [session_name]
#
# odom_source:
...
ROBOT_IP="${1:-10.0.2.7}"
ODOM_SOURCE="${2:-rtabmap}"
SESSION="${3:-rbpodo_slam_real}"
WS="$HOME/ros2_ws"
...
# Pane 0 (top-left): robot bringup
tmux send-keys -t "$SESSION:0.0" \
  "$SETUP && ros2 launch rbpodo_bringup rbpodo.launch.py \
  robot_ip:=${ROBOT_IP} model_id:=rb10_1300e" Enter
...
echo "Session '${SESSION}' started. Robot IP: ${ROBOT_IP}, odom_source: ${ODOM_SOURCE}"
